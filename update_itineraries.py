...
with app.app_context():
    ...
    count = 0
    for pkg in packages:
        if pkg.name in itineraries:
            pkg.itinerary = itineraries[pkg.name]
            count += 1
        elif "Istanbul" in pkg.name:
            pkg.itinerary = itineraries["Magical Istanbul"]
            count += 1
            
    db.session.commit()
    print(f"Successfully updated itineraries for {count} packages.")
